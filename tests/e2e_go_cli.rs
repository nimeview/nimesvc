...
#[test]
fn go_cli_generate_run_stop_e2e() {
...
"#
    ...
    assert!(out_dir.join("Api").join("go.mod").exists());
    assert!(out_dir.join("Api").join("main.go").exists());
    ...
    if !wait_for_http_ok(port, Duration::from_secs(30)) {
        ...
        panic!("Go service did not become healthy, run status: {status}");
    }
